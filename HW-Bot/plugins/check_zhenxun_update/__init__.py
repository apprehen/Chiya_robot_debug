...
__plugin_usage__ = """
usage：
    检查更新Chiya最新版本，包括了自动更新
    指令：
        检查更新Chiya
        重启
""".strip()
__plugin_des__ = "就算是Chiya也会成长的"
__plugin_cmd__ = ["检查更新Chiya", "重启"]
...
update_zhenxun = on_command("检查更新Chiya", permission=SUPERUSER, priority=1, block=True)
...
async def _(bot: Bot, event: MessageEvent):
    try:
        ...
        if error:
            logger.error(f"更新Chiya未知错误 {error}")
            await bot.send_private_msg(
                user_id=event.user_id, message=f"更新Chiya未知错误 {error}"
            )
    except Exception as e:
        logger.error(f"更新Chiya未知错误 {type(e)}：{e}")
        await bot.send_private_msg(
            user_id=event.user_id,
            message=f"更新Chiya未知错误 {type(e)}：{e}",
        )
    else:
        if code == 200:
            await bot.send_private_msg(
                user_id=event.user_id, message=f"更新完毕，请重启Chiya...."
            )
